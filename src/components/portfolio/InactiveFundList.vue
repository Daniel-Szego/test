        ...
        <div class="card">
            <header class="flex items-center mb-5">
                <div class="bg-white rounded shadow px-3 py-2 mr-3">
                    <h1 class="text-dark-grey-blue-two text-lg font-bold">Inactive Funds</h1>
                </div>

                <div class="flex-1 flex items-center justify-between bg-white rounded shadow px-1 py-1">
                    <span class="badge bg-aqua-marine rounded py-1 px-2 text-white text-base font-extrabold leading-none">{{ funds.length }}</span>
            ...
            <main v-if="current" class="bg-white rounded shadow p-3">
                <transition name="fade" mode="out-in">
                    <portfolio-inactive-fund :fund="current" :key="index"></portfolio-inactive-fund>
                </transition>
            </main>
        </div>
    </section>
...
<script>
    import PortfolioInactiveFund from "./InactiveFund";

    export default {
        components: {
            PortfolioInactiveFund
        },
